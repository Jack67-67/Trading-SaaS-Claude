...
import {
  ...
  TriangleAlert, Layers2, Activity,
...
// ── Empty state (module-level — returned early, no stats code runs) ───────────
...
export default async function PortfolioPage() {
  ...
  try {
    ...
    rows = strategies.map((s) => {
      type RunEntry = {
        id: string; status: string; results: unknown;
        completed_at: string | null; config: unknown;
      };
      const allRuns = (
      ...
      const latestMetrics = latestRun
        ? (((latestRun.results as Record<string, unknown>)?.metrics) as BacktestMetrics | null | undefined) ?? null
        : null;
      ...
      const snapshots = completed.flatMap((r) => {
        const m = ((r.results as Record<string, unknown>)?.metrics) as BacktestMetrics | null | undefined;
        if (!m) return [];
        return [{
      ...
      const alertInputs = completed.flatMap((r) => {
        ...
        return [{
          ...
          strategyName: (s.name as string) || "Unnamed",
      ...
      const prevMetrics = prevRun
        ? (((prevRun.results as Record<string, unknown>)?.metrics) as BacktestMetrics | null | undefined) ?? null
        : null;
      const returnDelta =
      ...
      return {
        ...
        trend,
        alerts: generateAlerts(alertInputs),
        lastSymbol,
    ...
    // fetch or row-building failed — rows stays []
  }

  // ── EARLY RETURN: no strategies ────────────────────────────────────────────
  if (rows.length === 0) {
    return <PortfolioEmpty />;
  }

  // ── Step 2: stats (only runs when rows exist) ──────────────────────────────
  ...
  const bestRow    = analyzed.length ? analyzed.reduce((a, b) => (a.metrics!.total_return_pct ?? 0) > (b.metrics!.total_return_pct ?? 0) ? a : b) : null;
  const worstRow   = analyzed.length ? analyzed.reduce((a, b) => (a.metrics!.total_return_pct ?? 0) < (b.metrics!.total_return_pct ?? 0) ? a : b) : null;
    ...
    <div className="space-y-6 animate-fade-in max-w-6xl">
      ...
      <div className="rounded-2xl border border-border bg-surface-1 overflow-hidden">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 divide-x divide-y lg:divide-y-0 divide-border">
          <StatCell label="Strategies" value={String(rows.length)} sub={`${analyzed.length} analyzed`} />
          {avgReturn !== null
            ? <StatCell label="Avg Return" value={formatPercent(avgReturn)} valueClass={pnlColor(avgReturn)} />
            : <StatCell label="Avg Return" value="—" sub="No data" />}
          {avgSharpe !== null
            ? <StatCell label="Avg Sharpe" value={avgSharpe.toFixed(2)} valueClass={avgSharpe >= 1 ? "text-profit" : avgSharpe >= 0.5 ? "text-amber-400" : "text-loss"} />
            : <StatCell label="Avg Sharpe" value="—" />}
          {bestRow
            ? <StatCell label="Best"  value={formatPercent(bestRow.metrics!.total_return_pct ?? 0)}  valueClass="text-profit" sub={bestRow.name} />
            : <StatCell label="Best"  value="—" />}
          {worstRow
            ? <StatCell label="Worst" value={formatPercent(worstRow.metrics!.total_return_pct ?? 0)} valueClass={pnlColor(worstRow.metrics!.total_return_pct ?? 0)} sub={worstRow.name} />
        ...
        {analyzed.length >= 2 && (
          <div className="px-5 py-3 border-t border-border flex items-center gap-6 flex-wrap">
            <p className="text-2xs font-semibold text-text-muted uppercase tracking-wider shrink-0">Trend</p>
            {trendCounts.improving > 0 && <TrendPill count={trendCounts.improving} label="Improving" cls="text-profit" />}
            {trendCounts.stable    > 0 && <TrendPill count={trendCounts.stable}    label="Stable"    cls="text-text-muted" />}
            {trendCounts.atRisk    > 0 && <TrendPill count={trendCounts.atRisk}    label="At-risk"   cls="text-yellow-400" />}
            {trendCounts.declining > 0 && <TrendPill count={trendCounts.declining} label="Declining" cls="text-loss" />}
            {Object.values(trendCounts).every((v) => v === 0) && (
      ...
      <div className="rounded-2xl border border-border overflow-hidden">
        <div className="hidden lg:grid lg:grid-cols-[minmax(0,2fr)_120px_80px_100px_120px_minmax(0,1fr)_40px] gap-4 px-5 py-2.5 bg-surface-1 border-b border-border">
          {["Strategy", "Return", "Sharpe", "Risk", "Trend", "Verdict", ""].map((h) => (
            <p key={h} className="text-2xs font-semibold text-text-muted uppercase tracking-wider">{h}</p>
          ))}
        </div>
...
function StatCell({ label, value, valueClass, sub }: {
  ...
  return (
    <div className="px-5 py-4">
      ...
      <p className={cn("text-xl font-bold font-mono tabular-nums", valueClass ?? "text-text-primary")}>{value}</p>
      {sub && <p className="text-2xs text-text-muted mt-0.5 truncate">{sub}</p>}
    </div>
  );
}

function TrendPill({ count, label, cls }: { count: number; label: string; cls: string }) {
  return (
...
function StrategyTableRow({ row }: { row: StrategyRow }) {
  ...
  let riskLabel: ReturnType<typeof generateRiskLabel> | null = null;
  let confidence: ReturnType<typeof computeConfidence> | null = null;
  if (metrics) {
    try { verdict    = generateVerdict(metrics);    } catch { /* skip */ }
    try { riskLabel  = generateRiskLabel(metrics);  } catch { /* skip */ }
    try { confidence = computeConfidence(metrics);  } catch { /* skip */ }
  }
  ...
  const sharpePct  = metrics?.sharpe_ratio ?? 0;
    ...
    >
      ...
      <div className="min-w-0 flex items-start gap-3 w-full lg:w-auto">
        <div className="shrink-0 mt-0.5">
          {critAlert  ? <CircleX    size={14} className="text-loss" />         :
           warnAlert  ? <TriangleAlert size={14} className="text-yellow-400" /> :
           metrics    ? <Activity   size={14} className="text-profit/60" />    :
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            <span className="text-2xs text-text-muted">
              {row.runCount > 0 ? `${row.runCount} ${row.runCount === 1 ? "run" : "runs"}` : "No runs yet"}
            </span>
            {row.lastAnalyzedAt && (
              <span className="text-2xs text-text-muted/50">· {timeAgo(row.lastAnalyzedAt)}</span>
            )}
          </div>
          {activeAlert && (
            <p className="text-2xs text-yellow-400 mt-1 line-clamp-1 lg:hidden">{activeAlert.title}</p>
          )}
        </div>
      </div>
        ...
        {metrics ? (
          <>
            <div className="flex items-center lg:justify-end gap-1">
              {returnPct >= 0.5  ? <TrendingUp   size={11} className="text-profit" />    :
               returnPct <= -0.5 ? <TrendingDown  size={11} className="text-loss" />     :
                                   <Minus         size={11} className="text-text-muted" />}
              <span className={cn("text-base font-bold font-mono tabular-nums", pnlColor(returnPct))}>
            ...
            {row.returnDelta !== null && (
              <p className={cn(
                ...
                row.returnDelta < -0.5 ? "text-loss"   : "text-text-muted/50"
              )}>
      ...
      <div className="w-full lg:w-auto">
        ...
        {metrics ? (
          <span className={cn("text-sm font-mono font-semibold tabular-nums",
            sharpePct >= 1 ? "text-profit" : sharpePct >= 0.5 ? "text-amber-400" : "text-loss"
          )}>
            {sharpePct.toFixed(2)}
          </span>
        ) : <span className="text-sm text-text-muted/40">—</span>}
      ...
      <div className="w-full lg:w-auto">
        ...
        {riskLabel ? (
          <span className={cn("text-2xs font-semibold px-2 py-1 rounded-full border", RISK_CLS[riskLabel.level] ?? "")}>
            {riskLabel.label}
          </span>
        ) : <span className="text-sm text-text-muted/40">—</span>}
      </div>
      ...
      <div className="w-full lg:w-auto min-w-0">
        ...
        {verdict ? (
          <div className="flex items-center gap-2 flex-wrap">
            <span className={cn("text-2xs font-bold px-2 py-1 rounded-full border leading-none", VERDICT_CLS[verdict.color] ?? "")}>
              {verdict.tagline}
            </span>
            {confidence && (
